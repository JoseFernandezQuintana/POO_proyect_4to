...
# Bordes discretos (configuración centralizada)
VISIBLE_BORDER = "#C8CDD6"  # gris suave, visible pero discreto
...
class AgendarCitaFrame(ctk.CTkFrame):
    ...
    def _obtener_rangos_edad(self):
        """Define los rangos de edad requeridos para el dropdown."""
        return [
        "Selecciona un rango",
        "0 - 3 años",
        "3 - 6 años",
    ...
    def create_form_panel(self):
        ...
        # --- MOVIMIENTO 1: Header del formulario (VA PRIMERO) ---
        ...
        # --- MOVIMIENTO 2: Contenedor/Wrapper del ScrollableFrame (VA DESPUÉS) ---
        # Este es el gran recuadro que deseas llenar con los campos.
        ...
        # Usamos expand=True para que ocupe todo el espacio restante debajo del header.
        scroll_wrapper.pack(fill="both", expand=True, padx=30, pady=(10, 30))
        
        # --- MOVIMIENTO 3: Contenedor desplazable para los campos (VA DENTRO DEL WRAPPER) ---
        # El ScrollableFrame que contiene toda la lógica de grid
        inner_form_frame = ctk.CTkScrollableFrame(scroll_wrapper, fg_color="transparent")
        # Aseguramos que el ScrollableFrame llene el wrapper
        inner_form_frame.pack(fill="both", expand=True, padx=0, pady=0)
        
        # Aseguramos columnas
        inner_form_frame.grid_columnconfigure((0, 1), weight=1)
        
        # Campos del formulario (El resto del código de campos se mantiene igual, 
        # ya que todos usan inner_form_frame como padre).
        ...
        # Edad (Implementación de selección por rangos)
        self.age_var = ctk.StringVar(value=self._obtener_rangos_edad()[0])
        self._add_form_dropdown(inner_form_frame, "Rango de Edad *", self._obtener_rangos_edad(), 0, 1, variable=self.age_var)
        ...
        # Contenedor del campo de Tratamiento Previo (NO USA GRID INICIALMENTE)
        ...
        self._add_form_dropdown(inner_form_frame, "Doctora encargada *", ["Selecciona una doctora"] + doctoras, 2, 1, variable=self.doctora_var)
        self._add_date_time_fields(inner_form_frame, 6) 
        ...
        # Contenedor de Información de Pago (NO USA GRID INICIALMENTE)
        self.payment_info_frame = ctk.CTkFrame(inner_form_frame, fg_color="transparent")
        
        self._add_notes_field(inner_form_frame, 12, offset=1) 
        self._add_reminder_section(inner_form_frame, 14, offset=1)
    ...
    # --- MÉTODO ELIMINADO: validate_age_input ---
    ...
    def toggle_prev_treatment(self, choice):
        ...
        if choice == "Sí":
            # Muestra el frame y su contenido
            ...
            ctk.CTkEntry(
                self.prev_treatment_frame,
                placeholder_text="Ej: Usé Brackets por 2 años",
                height=35,
                fg_color=WHITE_FRAME,
                border_color=VISIBLE_BORDER,
                border_width=DEFAULT_BORDER_WIDTH
            ...
            # Oculta el frame y libera la fila 4
    ...
    def toggle_payment_info(self, choice):
        ...
        if choice == "Tratamiento (Con costo)":
            ...
            # Oculta el frame y libera la fila 12
    ...
    def _add_payment_fields(self, parent):
        ...
        payment_card = ctk.CTkFrame(parent, fg_color="#E9F5FF", corner_radius=10, border_color=VISIBLE_BORDER, border_width=DEFAULT_BORDER_WIDTH)
        ...
        ctk.CTkEntry(payment_card, placeholder_text="Ej: 5000", height=30, fg_color="white", border_color=VISIBLE_BORDER, border_width=DEFAULT_BORDER_WIDTH).grid(row=2, column=0, sticky="ew", padx=(15, 10), pady=(0, 15))
        ...
        ctk.CTkEntry(payment_card, placeholder_text="Ej: 1000", height=30, fg_color="white", border_color=VISIBLE_BORDER, border_width=DEFAULT_BORDER_WIDTH).grid(row=2, column=1, sticky="ew", padx=(10, 15), pady=(0, 15))
        ...
        tratamientos_frame = ctk.CTkFrame(payment_card, fg_color="transparent", border_color=VISIBLE_BORDER, border_width=DEFAULT_BORDER_WIDTH)
        ...
        ctk.CTkEntry(tratamientos_frame, placeholder_text="Limpieza dental, Extracción", height=30, fg_color="white", border_color=VISIBLE_BORDER, border_width=DEFAULT_BORDER_WIDTH).grid(row=0, column=0, sticky="ew", padx=(0, 5))
    ...
    def _add_form_field(self, parent, label_text, placeholder, row, col, columnspan=1, validate_age=False):
        # Se ignora validate_age, ya que la edad se maneja con dropdown ahora.
        ...
        # Se elimina la lógica de validación de edad
        entry = ctk.CTkEntry(
        parent, 
    ...
    def _add_form_dropdown(self, parent, label_text, values, row, col, columnspan=1, variable=None, command=None):
        ctk.CTkLabel(parent, text=label_text, font=ctk.CTkFont(size=14, weight="bold"), anchor="w", text_color="#333333").grid(row=row, column=col, sticky="w", pady=(15, 5), padx=(0, 10))
        # OptionMenu no tiene border_color, así que lo envolvemos en un frame con borde
    ...
    def _add_date_time_fields(self, parent, row):
        ...
        # CONFIGURACIÓN PARA ACOPLAR EL BOTÓN Y EL ICONO
        date_frame.grid_columnconfigure(1, weight=1) # Columna del botón toma el espacio

        # Icono (Columna 0)
        icon_label = None
        try:
            icon_calendar = ctk.CTkImage(Image.open(ICON_CALENDAR_PATH).resize((18, 18)), size=(18, 18))
            icon_label = ctk.CTkLabel(date_frame, text="", image=icon_calendar)
        except:
            icon_label = ctk.CTkLabel(date_frame, text="📅", font=ctk.CTkFont(size=16))

        icon_label.grid(row=0, column=0, padx=(10, 5), pady=0, sticky="w")
        ...
        fg_color=WHITE_FRAME, 
        hover_color=SOFT_BLUE_FRAME, 
        text_color="#6B6B6B", 
        font=ctk.CTkFont(size=14, weight="normal"), 
        # Se establece un ancho mínimo y se usa sticky="ew" para que se expanda
        width=1, 
        height=33
        ...
        # Envolvemos la OptionMenu de hora para que tenga borde visible
        ...
        ctk.CTkOptionMenu(
        time_wrapper, values=["Seleccionar hora"], height=35, fg_color=WHITE_FRAME, button_color=WHITE_FRAME, 
    ...
    def _add_reminder_section(self, parent, row, offset=0):
        ...
        reminder_toggle_frame = ctk.CTkFrame(parent, fg_color=WHITE_FRAME, corner_radius=10, border_color=VISIBLE_BORDER, border_width=DEFAULT_BORDER_WIDTH)
        ...
        # ❌ ELIMINAMOS EL GRID INICIAL AQUÍ
    ...
    def toggle_contact_info(self):
        ...
        # Determinamos la fila para que el contenedor se coloque justo después del switch.
        # El switch está en la fila 'row + offset', por lo tanto, el contenedor de contacto 
        # va a la fila 'row + 1 + offset'. (Se usa el mismo esquema de filas que en _add_reminder_section)
        parent = self.contact_info_container.master
        row = 14 # Fila base
        offset = 1 # Offset base

        if self.reminder_switch.get() == 1:
            # AÑADIDO: Muestra el frame de contacto
            ...
            # AÑADIDO: Oculta el frame de contacto y libera el espacio
    ...
    def _add_contact_fields(self, parent):
        ...
        contact_info_card = ctk.CTkFrame(parent, fg_color="#E9F5FF", corner_radius=10, border_color=VISIBLE_BORDER, border_width=DEFAULT_BORDER_WIDTH)
        ...
        ctk.CTkEntry(contact_info_card, placeholder_text="Ej: 3331234567", height=30, border_width=DEFAULT_BORDER_WIDTH, border_color=VISIBLE_BORDER, fg_color="white").grid(row=2, column=0, sticky="ew", padx=(20, 10), pady=(0, 10))
        ...
        ctk.CTkEntry(contact_info_card, placeholder_text="ejemplo@correo.com", height=30, border_width=DEFAULT_BORDER_WIDTH, border_color=VISIBLE_BORDER, fg_color="white").grid(row=2, column=1, sticky="ew", padx=(0, 20), pady=(0, 10))
    ...
    def _add_doctor_info(self, parent, name, specialty):
        # **CORRECCIÓN 1: Se elimina el borde del recuadro individual de la doctora (border_width=0)**
        doctor_frame = ctk.CTkFrame(parent, fg_color="transparent", border_color=VISIBLE_BORDER, border_width=0, corner_radius=8)
    ...
    def _add_my_appointments_card(self, parent, row):
        ...
        no_citas_frame = ctk.CTkFrame(card, fg_color=SOFT_BLUE_FRAME, corner_radius=10, border_color=VISIBLE_BORDER, border_width=DEFAULT_BORDER_WIDTH)
